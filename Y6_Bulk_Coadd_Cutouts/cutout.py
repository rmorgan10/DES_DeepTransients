...
class CutoutProducer:
    ...
    def __init__(self, field, ccd, season, cutout_size, outdir, catalog_filename, test=False, maglim=90,
    ...
    def read_catalog(self):
        ...
        """
        filename = self.catalog_path + self.catalog_filename
        if not os.path.exists(filename):
            raise IOError(f"{filename} not found")
    ...
    def read_image(self, filename):
        ...
        # Protect against corrupted images by triggering automatic re-download.
        try:
            image = f["SCI"].data
        except TypeError:
            print(f"Corrupted file {filename} detected, redownloading...")
            f.close()
            redownload(filename)
            f = fits.open(filename, mode='readonly')
            image = f["SCI"].data
        ...
        # image = np.pad(image, self.pad, mode='constant', constant_values=image.min())
    ...
    def cutout_objects(self, image, wcs):
        ...
        for i, (x, y) in enumerate(zip(object_x, object_y)):
            ...
            too_close_to_edge = (
                (
                    x < self.cutout_size // 2 + 1 or 
                    x > np.shape(image)[1] - (self.cutout_size // 2 + 1)
                ) or 
                (
                    y < self.cutout_size // 2 + 1 or 
                    y > np.shape(image)[0] - (self.cutout_size // 2 + 1)
                )
            )
    ...
    def combine_bands(self, g_filename, r_filename, i_filename, z_filename):
        ...
        # If any missing bands on a nite are detected, fill with NaN
        for idx in missing_idx:
            image_array[idx] = np.nan
...
if __name__ == "__main__":
    ...
    parser.add_argument('--catalog', 
                        type=str,
                        default='deep_catalog.csv',
                        help='Name of catalog file')
    
    args = parser.parse_args()

    # Make a CutoutProducer for the tile
    cutout_prod = CutoutProducer(
        args.field, args.ccd, args.season, args.size, args.outdir, args.catalog,
        test=args.test, maglim = args.maglim)
